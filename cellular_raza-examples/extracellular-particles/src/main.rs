...
use nalgebra::{Matrix4xX, Vector2};
...
pub const N_TIMES: u64 = 30_000;
pub const SAVE_INTERVAL: u64 = 10;

pub const DOMAIN_SIZE: f64 = 150.0;
...
fn reflect_at(
    ...
    particle: &mut nalgebra::SVectorViewMut<f64, 4>,
    // Stützpunkt
    q: &nalgebra::SVectorView<f64, 2>,
    // Normal pointing from q into the prohibited space
    dir: &nalgebra::SVectorView<f64, 2>,
) -> nalgebra::SVector<f64, 2> {
    let pos = particle.view((0, 0), (2, 1));
    ...
    let mut pos = particle.view_mut((0, 0), (2, 1));
    pos.sub_assign(2.0 * d * dir);

    let mut vel = particle.view_mut((2, 0), (2, 1));
...
impl Cycle for Cell {
    ...
    fn divide(rng: &mut rand_chacha::ChaCha8Rng, cell: &mut Self) -> Result<Self, DivisionError> {
        ...
        let dir = Vector2::from([phi.cos(), phi.sin()]);
        ...
        let n = cell.particles.ncols();
        let d1 = n.div_ceil(2);
        let d2 = n - d1;

        let mut pa1 = ParticleVec::zeros(n);
        for i in 0..d1 {
            pa1.set_column(i, &cell.particles.column(i));
        }

        let mut pa2 = ParticleVec::zeros(n);
        for j in 0..d2 {
            pa2.set_column(j, &cell.particles.column(j + d1));
        }
...
/// v[4] = vel1;
/// ```
type ParticleVec<F = f64> = Matrix4xX<F>;
...
impl Intracellular<ParticleVec> for Cell {
    ...
    fn set_intracellular(&mut self, intracellular: ParticleVec) {
        ...
        for n in 0..self.particles.ncols() {
            let pos = self.particles.view((0, n), (2, 1));
            ...
            if d >= 0.0 {
                ...
                self.mechanics.vel += vel_change * P_MASS / self.mechanics.mass;
            }
        }
...
impl Reactions<ParticleVec> for Cell {
    fn calculate_intracellular_increment(
        ...
        dintra.swap_rows(0, 2);
        dintra.swap_rows(1, 3);

        let mut dvel = dintra.view_mut((2, 0), (2, intracellular.ncols()));
        dvel *= 0.0;
...
#[derive(Clone)]
struct Cuboid2D<F, const D: usize> {
    base: CartesianCuboid<F, D>,
    particles: ParticleVec<F>,
}

impl<F, C, Ci, const D: usize> Domain<C, CuboidSubDomain<F, D>, Ci> for Cuboid2D<F, D>
where
    ...
    F: 'static + num::Float + core::fmt::Debug + core::ops::SubAssign + core::ops::DivAssign,
{
    ...
    {
        let Cuboid2D { base, particles } = self;
...
impl SubDomainReactions<Vector2<f64>, ParticleVec, f64> for CuboidSubDomain<f64, 2> {
    ...
    where
        ...
        J: IntoIterator<Item = (Vector2<f64>, ParticleVec)>,
    {
        // Bounce off cells
        Ok(())
    }
    ...
    fn update_fluid_dynamics(&mut self, dt: f64) -> Result<(), cellular_raza::concepts::CalcError> {
        ...
        for n in 0..self.particles.ncols() {
            for i in 0..2 {
                self.particles[(i, n)] += dt * self.particles[(i + 2, n)];
            }

            // Put in new voxel
            let index = self
                .base
                .get_index_of([self.particles[0], self.particles[1]])
                .map_err(|e| cellular_raza::prelude::CalcError(e.to_string()))?;

            // Handle particle-cell collisions

            // Apply boundary conditions
            let mut pos = [self.particles[(0, n)], self.particles[(1, n)]];
            let mut vel = [self.particles[(2, n)], self.particles[(3, n)]];
            self.base
            ...
            for i in 0..2 {
                ...
                self.particles[(i + 2, n)] = vel[i];
            }
        }
        Ok(())
    }

    fn get_extracellular_at_pos(&self, pos: &Vector2<f64>) -> Result<ParticleVec, CalcError> {
        let index = self
            .base
            .get_index_of([pos[0], pos[1]])
        ...
        .map(|i| {
            ...
            Cell {
                mechanics: NewtonDamped2D {
                    pos,
                    vel: Vector2::from([
                        rng.random_range(-C_VELOCITY_INIT_BOUND..C_VELOCITY_INIT_BOUND),
                        rng.random_range(-C_VELOCITY_INIT_BOUND..C_VELOCITY_INIT_BOUND),
                    ]),
                    damping_constant: C_DAMPING,
                    mass: 0.3,
                },
                interaction: MorsePotential {
                    radius: C_RADIUS,
                    potential_stiffness: 0.2,
                    strength: C_POTENTIAL_STRENGTH,
...
fn test_reflect_at_1() {
    ...
    assert!(particle[2] == -1.0);
    assert!(particle[3] == 0.0);
}
...
fn test_reflect_at_2() {
    ...
    assert!((particle[2] + 0.5).abs() < 1e-8);
    assert!((particle[3] + 1.0).abs() < 1e-8);
}
