...
/// <script>
/// function ljp(x, epsilon, sigma, bound) {
///     var s = 4*epsilon*((sigma/x)**12 - (sigma/x)**6);
///     return Math.min(bound, s);
/// }
/// function generate_data(x, epsilon, sigma, bound) {
///     var y = [];
///     x.forEach((xi, i) => {
///         y[i] = ljp(xi, epsilon, sigma, bound);
///     });
///     return y;
/// }
/// var epsilon = 1.0;
...
/// var n_samples = 200;
/// var x = [...Array(n_samples).keys()].map((x) => 3*x/(sigma*n_samples));
/// var y = generate_data(x, epsilon, sigma, bound);
/// var trace = {
///     x: x,
///     y: y,
///     type: 'scatter',
...
/// var layout = {
///     xaxis: {
///         tickvals: [sigma, 2**(1/6)*sigma, 2*sigma],
///         ticktext: ['σ', "1,12σ", '2σ'],
///         color: "#FFF",
///     },
///     yaxis: {
...
pub struct BoundLennardJones<const D: usize> {
    ...
    pub sigma: f64,
    /// Numerical bound of the interaction. This will determine the maximum force
    /// which can be generated by the potential.
    pub bound: f64,
}
