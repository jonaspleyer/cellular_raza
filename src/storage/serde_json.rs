...
pub struct JsonSaveFormat<Id, Element> {
    identifier: Id,
    element: Element,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct JsonBatchSaveFormat<Id, Element> {
    data: Vec<JsonSaveFormat<Id, Element>>,
}
...
impl<Id, Element> JsonStorageInterface<Id, Element> {
    fn create_or_get_iteration_file_with_prefix(
        ...
        iteration: u64,
        prefix: &str,
    ) -> Result<std::io::BufWriter<std::fs::File>, SimulationError> {
...
impl<Id, Element> StorageInterface<Id, Element> for JsonStorageInterface<Id, Element> {
        ...
        Element: Serialize,
    {
        ...
        let save_format = JsonSaveFormat {
            identifier,
            element,
        };
