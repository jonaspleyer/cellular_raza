...
fn main() {
    ...
    let mut cells = (0..N_CELLS_ORGANOID as i32).map(|_| {
    ...
    let mut cells2 = (0..N_CELLS_SENDER).map(|_| {
    ...
    let t_eval = (0..N_TIMES).map(|n| (T_START + n as f64 * DT, n % SAVE_INTERVAL == 0, n % FULL_SAVE_INTERVAL == 0)).collect::<Vec<(f64, bool, bool)>>();
