...
pub type MyCellType = ModularCell<MechanicsModel2D, CellSpecificInteraction, OwnCycle, OwnReactions, GradientSensing>;
...
impl Cycle<MyCellType> for OwnCycle {
    ...
    ) -> Option<CycleEvent>
    {
        ...
        if
            ...
            c.cellular_reactions.get_intracellular()[2] < 250.0
        {
...
impl CellularReactions<ReactionVector> for OwnReactions {
    ...
    ) -> Result<(ReactionVector, ReactionVector), CalcError>
    {
        ...
        let bmp4 = internal_concentration_vector[0];
        increment_extracellular[2] = self.p1 - self.p2*a + self.p3*a.powf(2.0)*b;
        increment_extracellular[3] = self.p4/(1.0+self.bmp4_hill * bmp4)  - self.p3*a.powf(2.0)*b;
