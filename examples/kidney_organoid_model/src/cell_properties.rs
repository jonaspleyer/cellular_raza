...
impl CellularReactions<ReactionVector> for OwnReactions {
    ...
    ) -> Result<(ReactionVector, ReactionVector), CalcError>
    {
        ...
        for i in 0..2 {
            let modifier_1 = (1.0 - internal_concentration_vector[i]/self.intracellular_concentrations_saturation_level[i]).clamp(0.0, 1.0);
            let modifier_2 =        internal_concentration_vector[i]/self.intracellular_concentrations_saturation_level[i].max(0.0);
            let uptake = - self.uptake_rate[i]*external_concentration_vector[i]*modifier_1;
            let secretion = self.secretion_rate[i]*(internal_concentration_vector[i]-self.intracellular_concentrations_saturation_level[i])*modifier_2;
        ...
        increment_intracellular[1] -= self.production_term[1].abs()*(external_concentration_vector[2]/4000.0).clamp(0.0,1.0);
