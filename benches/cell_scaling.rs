...
fn cell_scaling(c: &mut Criterion) {
    let mut group = c.benchmark_group("cell_sorting");

    for n_cells in [10, 50, 100, 500, 1000, 5000].into_iter() {
        group.bench_with_input(
            BenchmarkId::new("cell_scaling", n_cells),
            &n_cells,
            |b, &n_cells| {
                b.iter(|| {
                    run_simulation(
                        ...
                        n_cells,
                        1,
                        3.0 + (n_cells as f64 / 500.0).powf(1.0 / 3.0),
                        10,
                        0.25,
                    )
                })
...
fn thread_scaling(c: &mut Criterion) {
    ...
    for n_threads in 1..9 {
        ...
        group.bench_with_input(
            BenchmarkId::new("thread_scaling", n_threads),
            &n_threads,
            |b, &n_threads| {
